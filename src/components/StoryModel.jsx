...
const StoryModel = ({setShowModel, fetchStories}) => {
  ...
  return (
    <div className='fixed inset-0 z- min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
            ...
            <div className='flex mt-4 gap-2'>
                {bgColors.map((color)=>(
                    <button key={color} className='w-6 h-6 rounded-full ring cursor-pointer' style={{background: color}} onClick={() => setBackground(color)} /> 
                ))}
            </div>
